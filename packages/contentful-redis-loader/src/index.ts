...
const createLoaders = (config: LastRevAppConfig, fallbackLoaders: CmsLoaders): CmsLoaders => {
  ...
  const getBatchEntriesByContentTypeFetcher =
    ({ entryLoader }: { entryLoader: DataLoader<ItemKey, any> }): DataLoader.BatchLoadFn<ItemKey, CmsEntry<any>[]> =>
    async (keys) => {
      ...
      const out: { [contentType: string]: CmsEntry<any>[] } = {};
      ...
      if (cacheMissContentTypeIds.length) {
        ...
        (filtered as CmsEntry<any>[][]).forEach((entryArray, idx) => {
