...
describe('PathToContentLoader', () => {
  describe('root', () => {
    it('loads correctly', async () => {
      const loaded = await pathLoader.getItemsForPath('/', mockApolloContext());
      expect(loaded).toEqual([entryMocks.homepage]);
    });
  });
  ...
  describe('full paths', () => {
    it('loads with a full path', async () => {
      const loaded = await pathLoader.getItemsForPath('/some/path/here', mockApolloContext());
      expect(loaded).toEqual([entryMocks.pageWithFullPath]);
    });
  });
});
