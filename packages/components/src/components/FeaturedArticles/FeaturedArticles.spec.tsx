...
describe('FeaturedArticles', () => {
  context('renders correctly', () => {
    it('renders a list of articles', () => {
      const mockedContent: FeaturedArticlesProps = { articles: [...articlesMock] };
      ...
      cy.get('[data-testid=FeaturedArticles-articleCard]').each((cardElement, index) => {
        cy.wrap(cardElement).find('[data-testid=Card-media]').should('exist').and('have.attr', 'src', mockedContent.articles[index].media.file.url);
        cy.wrap(cardElement).find('[data-testid=Card-title]').should('exist').and('have.text', mockedContent.articles[index].title);
