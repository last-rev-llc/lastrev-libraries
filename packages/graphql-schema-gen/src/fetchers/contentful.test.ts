...
describe('generateContentfulSchema', () => {
  ...
  describe('field type mapping', () => {
    ...
    it('should map Symbol fields to String', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Symbol' })], false);
      expect(result).toContain('testField: String');
    });

    it('should map Text fields to String', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Text' })], false);
      expect(result).toContain('testField: String');
    });

    it('should map Integer fields to Int', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Integer' })], false);
      expect(result).toContain('testField: Int');
    });

    it('should map Number fields to Float', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Number' })], false);
      expect(result).toContain('testField: Float');
    });

    it('should map Date fields to Date', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Date' })], false);
      expect(result).toContain('testField: Date');
    });

    it('should map Location fields to Location', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Location' })], false);
      expect(result).toContain('testField: Location');
    });

    it('should map Object fields to JSON', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Object' })], false);
      expect(result).toContain('testField: JSON');
    });

    it('should map Boolean fields to Boolean', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'Boolean' })], false);
      expect(result).toContain('testField: Boolean');
    });

    it('should map RichText fields to RichText', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ id: 'testField', name: 'Test', type: 'RichText' })], false);
      expect(result).toContain('testField: RichText');
    });

    it('should map Link Asset fields to Media', () => {
      const result = generateContentfulSchema({}, [createContentTypeWithField({ 
        id: 'testField', 
        name: 'Test', 
        type: 'Link', 
        linkType: 'Asset'
    ...
    it('should skip reference fields when skipReferenceFields is true', () => {
      const contentTypes: ContentType[] = [
        createContentType('article', 'Article', [
          ...
          createField({ id: 'relatedArticles', name: 'Related', type: 'Array', items: { type: 'Link', linkType: 'Entry' } })
        ])
      ];
