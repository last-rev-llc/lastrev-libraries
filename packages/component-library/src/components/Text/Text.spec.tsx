    ...
    describe('node types', () => {
      ...
      context('other node types', () => {
        it('renders text with embedded inline entry', () => {
          ...
          const mockedContent: TextProps = dynamicMock([ contentNode([ embeddedEntryInlineNode(mockedEntry.id) ]) ], [mockedEntry]);
          mount(<Text {...mockedContent} />);
        ...
        it('renders text with embedded asset block', () => {
          ...
          mount(<Text {...mockedContent} />);
          cy.get('[data-testid=Text-embedded-asset-block]').should('exist').and('have.attr', 'src', mockedMedia.file.url);
          cy.percySnapshot();
        });
