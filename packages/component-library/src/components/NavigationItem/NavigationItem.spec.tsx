...
describe('NavigationItem', () => {
  context('renders correctly', () => {
    it('renders a navigation item and its subnav links', () => {
      ...
      cy.get('a').should('have.length', mockedContent.subNavigation.length + 1).each((item, index) => {
        if (index !== 0) {
          cy.wrap(item).should('have.attr', 'href', `/${mockedContent.subNavigation[index - 1].href}`)
            .and('have.text', mockedContent.subNavigation[index - 1].text);
        }
        else {
          cy.wrap(item).should('have.attr', 'href', `/${mockedContent.href}`)
