...
describe('PathParser', () => {
  it('throws an error when no string follows a colon', () => {
    ...
    expect(() => parser.PathRule()).toThrowError('Unexpected end of input. Expected IDENTIFIER');
  });

  it('throws an error when segment has no content', () => {
    const parser = new PathRuleParser(`/blogs//:slug`);

    expect(() => parser.PathRule()).toThrowError(`Unexpected token: /, expected one of [':', 'Field']`);
  });

  it('throws an error when path does not start with slash', () => {
    const parser = new PathRuleParser(`blogs/:slug`);

    expect(() => parser.PathRule()).toThrowError(`Unexpected Token: blogs, expected /`);
  });

  it('throws an error when reference expression has no contentType', () => {
    const parser = new PathRuleParser(`/:categories().slug`);

    expect(() => parser.PathRule()).toThrowError(`Unexpected Token: ), expected IDENTIFIER`);
  });
  ...
  it('throws an error when reference expression has too many strings', () => {
    ...
    expect(() => parser.PathRule()).toThrowError(`Unexpected Token: ,, expected )`);
  });
  ...
  it('throws an error when segment expression has no index', () => {
    ...
    expect(() => parser.PathRule()).toThrowError(`Unexpected Token: ), expected NUMBER`);
  });
  ...
  it('throws an error when segment expression has a non-number value', () => {
    ...
    expect(() => parser.PathRule()).toThrowError(`Unexpected Token: hello, expected NUMBER`);
  });
  ...
  it('returns an error when segment references exist with cycles', () => {
    ...
    expect(() => parser.PathRule()).toThrowError(`Cycle detected between segments 1 and 0`);
  });
  ...
  it('throws an error when segment references exist with indirect cycles', () => {
    ...
    expect(() => parser.PathRule()).toThrowError(`Cycle detected between segments 2 and 0`);
  });
