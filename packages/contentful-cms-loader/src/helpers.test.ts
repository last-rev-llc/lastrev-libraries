...
describe('helpers', () => {
  ...
  describe('makeContentfulRequest', () => {
    it('should fetch all items when total fits in one request', async () => {
      ...
      const result = await makeContentfulRequest(
        mockClient as any,
        'getEntries',
        100,
        { content_type: 'blog' }
    ...
    it('should recursively fetch all items when pagination is needed', async () => {
      ...
      const mockItems2 = [
        { sys: { id: 'item3' }, fields: { title: 'Item 3' } }
      ];
    ...
    it('should work with getAssets command', async () => {
      const mockAssets = [
        { sys: { id: 'asset1' }, fields: { file: { url: 'https://example.com/image1.jpg' } } }
      ];
    ...
    it('should use custom skip and existing parameters', async () => {
      const existingItems = [
        { sys: { id: 'existing1' }, fields: { title: 'Existing 1' } }
      ];
      const newItems = [
        { sys: { id: 'item1' }, fields: { title: 'Item 1' } }
      ];
