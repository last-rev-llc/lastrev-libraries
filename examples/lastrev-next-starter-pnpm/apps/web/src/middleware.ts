...
export function middleware() {
  const cspHeader = `
    ...
    script-src 'self' 'unsafe-inline' 'unsafe-eval' ${process.env.NEXT_PUBLIC_MARKETO_BASE_URL}  *.google-analytics.com *.googletagmanager.com vitals.vercel-insights.com *.hs-analytics.net *.hs-banner.com *.hsadspixel.net *.hscollectedforms.net *.jquery.com *.hs-scripts.com *.hsforms.net;
    font-src 'self'  *.sentry.io  fonts.gstatic.com *.hs-scripts.com *.hsforms.net data:;
    frame-src  https://forms.hsforms.com ${process.env.NEXT_PUBLIC_MARKETO_BASE_URL};
    img-src * data:;
    media-src * data:;
    object-src 'none';
    base-uri 'self';
