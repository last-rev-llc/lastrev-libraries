...
if (missingVars.length > 0) {
  ...
  console.log(
    "\x1b[36m%s\x1b[0m",
    'NOTE: Running "pnpm env:pull" will override all local .env files.'
  );
  console.log(
    "Please ensure to backup your .env file before running this command if necessary."
  );
  ...
  const timer = setTimeout(() => {
    ...
    prompt.close();
    console.log(
      "Exiting build process. Please set the required environment variables."
    );
    process.exit(1);
  }, 15000);
    ...
    (answer) => {
      ...
      if (answer.toLowerCase() === "yes") {
        try {
          ...
          console.error(
            "Failed to pull environment variables from envkey:",
            error
          );
          console.log(
            "Please address the above issue and rerun your original command."
          );
        ...
        console.log(
          "Please set the required environment variables manually and rerun your original command."
        );
  ...
  console.log(
    "All required environment variables are set. Continuing build..."
  );
}
